...
// Trizeps8: Connected to \RESET_OUT
static iomux_v3_cfg_t const wdog_pads[] = {
	IMX8MQ_PAD_GPIO1_IO02__WDOG1_WDOG_B | MUX_PAD_CTRL(WDOG_PAD_CTRL),
};
#endif

#if TRIZEPS8_CONTROL_RESET_OUT
static iomux_v3_cfg_t defaultpins_pads[] = {
	IMX8MQ_PAD_GPIO1_IO02__GPIO1_IO02 | MUX_PAD_CTRL(NO_PAD_CTRL),
};
#endif

int board_defaultpins_init(void)
{
#if TRIZEPS8_CONTROL_RESET_OUT
	imx_iomux_v3_setup_multiple_pads(defaultpins_pads,
					 ARRAY_SIZE(defaultpins_pads));

	// assert RESET_OUT (should already be pulled low):
	gpio_request(IMX_GPIO_NR(1, 2), "reset_out");
...
// Trizeps8: Pins routed as QSPI_xxx to SODIMM
...
// Trizeps8: UART1 pins routed to FFUART (same as i.MX8M EVK)
...
// Trizeps8:	Same Ethernet-Phy & Pins as i.MX8M EVK
...
int board_init(void)
{
	...
	gd->ram_size = get_ram_size((void *)CONFIG_SYS_SDRAM_BASE,
				    (ulong)PHYS_SDRAM_SIZE);

	printf("[TR8] RAM-size is 0x%llx\r\n", gd->ram_size);
	return 0;
}
