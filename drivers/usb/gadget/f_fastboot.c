...
int do_boota(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[]) {
	...
	printf("Kernel   @ %08x-%08x (%d)\n", hdr->kernel_addr,  hdr->kernel_addr + hdr->kernel_size,  hdr->kernel_size);
#ifdef CONFIG_OF_LIBFDT
	if (is_load_fdt_from_part()) {
		if (fdt_size)
	...
	char boot_addr_start[64];
	char fdt_addr[64];
	char ramdisk_addr[64];
...
#if defined(CONFIG_SYSTEM_RAMDISK_SUPPORT) && 0
	if (!is_recovery_mode)
		boot_args[2] = NULL;
#endif
...
static void cb_flashing(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (endswith(cmd, "lock_critical")) {
        ...
        } else if (endswith(cmd, "disable_dm_verity")) {
	                u32 disable_dm_verity=1;
			env_set_ulong("disable_dm_verity", disable_dm_verity);
			env_save();			  			
			strcpy(response, "OKAY");		       
	} else if (endswith(cmd, "enable_dm_verity")) {
	                u32 disable_dm_verity=0;
			env_set_ulong("disable_dm_verity", disable_dm_verity);
			env_save();			  
			strcpy(response, "OKAY");		       
	}else
